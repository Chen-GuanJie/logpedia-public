...
class STCedge:
    ...
    def __ne__(self, other):
        if not (isinstance(other, STCedge)):
            return True
...
def calc_H_G(G, pred, dst):
    ...
    nx.set_node_attributes(
        R, [], "H_G"
    )  # set H_G of each node to [] so we can later merge them
    h = R.nodes[dst]["H_G"]
        ...
        P.add_edges_from(
            [
                (
                    h[i],
                    h[2 * i + 1],
                    {"weight": (h[2 * i + 1].strc - h[i].strc), "cross_edge": False},
                )
            ]
        )  # For edge (u, v) in D(G), add as edge weight: d(v) - d(u)
        ...
        P.add_edges_from(
            [
                (
                    h[i],
                    h[2 * i + 2],
                    {"weight": (h[2 * i + 2].strc - h[i].strc), "cross_edge": False},
                )
            ]
        )
...
def HoutHeap_DFS(P, h, i, G):
    ...
    if 2 * i + 1 < len(h):
        c1 = h[2 * i + 1].root
        P.add_edges_from(
            [(p, c1, {"weight": (c1.strc - p.strc), "cross_edge": False})]
        )  # For edge (u, v) in D(G), add as edge weight: d(v) - d(u)
        ...
        P.add_edges_from(
            [
                (
                    ...
                    h[i].heap[0],
                    {
                        "weight": (h[i].heap[0].strc - h[i].root.strc),
                        "cross_edge": False,
                    },
                )
            ]
...
def prepare_and_augmentP(P, G, src):
    ...
    if h_s == []:
        ...
        h_sr = h_s[0].root
        P.add_edges_from(
            [(empty_seq, h_sr, {"weight": h_sr.strc, "cross_edge": False})]
        )
...
class EHeapElement:
    ...
    def __ne__(self, other):
        if not (isinstance(other, EHeapElement)):
            return True
        if not (self.__class__ == other.__class__):
            return True
        return not (self.__dict__ == other.__dict__)
...
def k_shortest_paths(G, src, dst, k):
    ...
    while len(paths) < k:
        ...
        while (
            False and Hroot.weight == last
        ):  # pop all sequences with same weight, for testing purposes, can be set to False for better performance
