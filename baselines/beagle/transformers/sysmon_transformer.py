...
class SysmonTransformer(Transformer):
    ...
    def transform(self, event: dict) -> Optional[Tuple]:
        ...
        if event_id == 1:
            ...
            return self.image_load(event)
        elif event_id==10:
            return self.process_access(event)
    ...
    def image_load(self,event:dict):
        ...
        file_image, file_path = split_path(event["EventData_ImageLoaded"])

        file_node = File(file_name=file_image, file_path=file_path)

        proc.loaded[file_node].append(timestamp=event["EventData_UtcTime"])

        return (proc, proc_file, file_node)
        
        return
    def process_access(self,event:dict):
        # Make the parent process
        parent_image, parent_path = split_path(event["EventData_SourceImage"])

        parent = SysMonProc(
            host=event["Computer"],
            process_id=int(event["EventData_SourceProcessId"]),
            process_guid=event["EventData_SourceProcessGUID"],
        ...
        process_image, process_path = split_path(event["EventData_TargetImage"])

        proc = SysMonProc(
            host=event["Computer"],
            process_guid=event["EventData_TargetProcessGUID"],
            process_id=int(event["EventData_TargetProcessId"]),
            process_image=process_image,
            process_image_path=process_path
        )
        ...
        parent.accessed[proc].append(timestamp=event["EventData_UtcTime"])
