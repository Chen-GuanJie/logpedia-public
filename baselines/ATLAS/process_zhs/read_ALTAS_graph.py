...
def read_nodes_edges(G,folder_name):
    ...
    with open(folder_name+'/edges.csv', 'w', newline='') as f:
        writer = csv.writer(f)
        # 写入 CSV 文件的标题行
    ...
    # 打开 CSV 文件以写入节点数据
    with open(folder_name+'/nodes.csv', 'w', newline='') as f:
        writer = csv.writer(f)
        # 写入 CSV 文件的标题行
...
if __name__ == "__main__":
    ...
    scene='S2'
    folder_name='ATLAS/process_zhs/'+scene+'/'
    file_name =folder_name+'process.csv'
    G=get_ATLAS_G(file_name)
    read_nodes_edges(G,folder_name)
